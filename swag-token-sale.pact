...
(define-keyset "free.bana-gov")
(define-keyset "free.bana-ops")

(module bana-sale GOV
  ;; -------------------------------
  ;; Governance and Permissions

  (defcap GOV ()
    (enforce-keyset "free.bana-gov")
    (compose-capability (OPS_INTERNAL))
  )

  (defcap OPS ()
    (enforce-keyset "free.bana-ops")
    (compose-capability (OPS_INTERNAL))
  )
  ...
  (defun update-fungible-bank-for-sale (sale:string guard:guard)
    @doc "Update the fungible bank guard for the sale."

    (with-capability (OPS)
      (update sales sale 
        { "fungible-bank-guard": guard
        , "fungible-bank-account": (create-principal guard) 
        }
      )
    )
  )
